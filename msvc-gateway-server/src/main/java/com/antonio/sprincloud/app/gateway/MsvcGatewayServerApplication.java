package com.antonio.sprincloud.app.gateway;
...
public class MsvcGatewayServerApplication {
	...
	RouterFunction<ServerResponse> routerConfig() {
		return route("mvsc-products")		
		.route(path("/api/products/**"), http())
		.filter((request, next) -> {
			ServerRequest requestModified = ServerRequest.from(request)
					.header("massage-request", "algun mensaje al request")
					.build();
			ServerResponse response = next.handle(requestModified);
			response.headers().add("massage-response", "algun mensaje para la respuesta");
			return response;
		})
		.filter(lb("mvsc-products"))
